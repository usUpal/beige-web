...
const Users = () => {
  ...
  const getShootDetails = async (shootId: string) => {
    setLoading(true);
    try {
      const response = await fetch(`https://api.beigecorporation.io/v1/cp/${shootId}`);
  ...
  const handleChange = (e: any) => {
    ...
    // Handle different input types (text, number, select, checkbox) appropriately
    const newValue = type === 'checkbox' ? e.target.checked : value;

    // Update the 'isData' state with the new value
  }
                    ...
                    <table>
                      ...
                      {allUsers?.map((user) => (
                        <tr key={user.id} className="group text-white-dark hover:text-black dark:hover:text-white-light/90">
                          ...
                          <td>
                            <button type="button" className="p-0" onClick={() => getShootDetails(user.userId.id)}>
                              ...
                              <form className="space-y-5" onSubmit={handleSubmit}>
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <div className="flex basis-[45%] flex-col sm:flex-row">
                                    ...
                                    <input id="userId" type="text" defaultValue={shootInfo?.id} className="form-input block font-sans" name='userId' onChange={handleChange} />
                                  </div>
                                </div>
                                <div className="flex items-center justify-between">
                                  {/* City */}
